...
export const fizzbuzz = (number) => {
    if (typeof number !== 'number') { throw new Error('Error especifico') };
    if (Number.isNaN(number)) { throw new Error('El numero es NaN') };

    const multiplies = { 3: 'fizz', 5: 'buzz' };
    let output = '';

    Object.entries(multiplies).map(([multiplier, word]) => {
